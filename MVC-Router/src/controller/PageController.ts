...
export class PageController {
  static getApiData(res: ServerResponse): void {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(
      JSON.stringify({
        message: `Manual route success, Some data from Model: ${Models.returnValue()}`,
      }),
    );
    return;
  }

  static mainPage(res: ServerResponse): void {
    res.writeHead(200, { 'Content-Type': 'text/html; charset=UTF-8' });
    res.end(MainView.getPage());
    return;
  }
